...
SELECT 
    ...
    COALESCE(
        json_agg(DISTINCT jsonb_build_object('whatsapp', bc.whatsapp_number)),
        '[]'
    ) AS contacts,
    json_agg(
        jsonb_build_object(
    ...
    (
        setweight(to_tsvector('simple', lower(coalesce(bp.business_name, ''))), 'A') ||
        setweight(to_tsvector('simple', lower(coalesce(bp.description, ''))), 'B') ||
        setweight(to_tsvector('simple', lower(string_agg(DISTINCT prov.name, ' '))), 'C') ||
        setweight(to_tsvector('simple', lower(string_agg(DISTINCT reg.name, ' '))), 'C') ||
        setweight(to_tsvector('simple', lower(string_agg(DISTINCT dist.name, ' '))), 'D')
    ) AS document,
...
SELECT 
    ...
    (
        setweight(to_tsvector('simple', lower(coalesce(s.name, ''))), 'A') ||
        setweight(to_tsvector('simple', lower(coalesce(s.description, ''))), 'B') ||
        setweight(to_tsvector('simple', lower(coalesce(c.name, ''))), 'B') ||
        setweight(to_tsvector('simple', lower(string_agg(DISTINCT prov.name, ' '))), 'C') ||
        setweight(to_tsvector('simple', lower(string_agg(DISTINCT reg.name, ' '))), 'C') ||
        setweight(to_tsvector('simple', lower(string_agg(DISTINCT dist.name, ' '))), 'D')
    ) AS document,
...
FROM search
WHERE 1=1 
AND ($1 = '' OR $1 IS NULL OR document @@ plainto_tsquery('simple',  lower($1))) 
