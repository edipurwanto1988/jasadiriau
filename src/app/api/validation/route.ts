...
import { createValidationSchema, updateValidationSchema } from "@/schema/validation.schema";
import { createValidation, updateValidation } from "@/http/services/validation.service";
...
export const POST = api(async (req) => {
  ...
  const validated = await createValidationSchema.parseAsync(payload);
  const session = await verifySession();
  if (!session) {
    return NextResponse.redirect(new URL("/login", req.nextUrl));
  }

  const result = await createValidation(validated);
