...
import { deletePage, getPages } from "@/views/services/page.service";
          ...
          onClick: () => {
            ...
            deletePage({ params: { id } })
              .then((resp) => {
                ...
                openSnackbar("Halaman berhasil dihapuskan.");
                alert.reset();
        ...
        column={[
          {
            label: "Judul",
            value: (value) => (
              <Link href={`/page/${value.slug}`} prefetch={false} target="_blank">
                {value.title}
              </Link>
            ),
            filter: {
